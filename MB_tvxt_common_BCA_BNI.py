...
# PDF2IMG_POPPLER_PATH=r"C:/Program Files/poppler-0.68.0/bin"
...
# def ConvertFileToImageByChris(filepath, DPI):
#     file_name = os.path.basename(filepath)
#     process_filepath = PROCESS_FOLDER_PATH + file_name
#     img,list_path = [],[]
#     if filepath.lower().endswith(('.png', '.jpg', '.jpeg','.JPG')):
...
def ProcessTesseract(image,psm,lang):
    if psm !='' and lang !='':
            image_to_string_result = pytesseract.image_to_string(image, lang=str(lang),config='--psm ' + str(psm) + " --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '")
    else :
        message = 'No Language or PSM parameter on Process Tesseract'
        PrintLog(message)
    # if z==1:
    #     image_to_string_result = pytesseract.image_to_string(image, lang="ind",config="--psm 6 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '")
    # elif z==2:
    #     image_to_string_result = pytesseract.image_to_string(image, lang="ind",config="--psm 3 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '")
    # elif z == 3 :
    #     #BNI TYPE 1
    #     image_to_string_result = pytesseract.image_to_string(image, lang="ind_w",config="--psm 6 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '")
    # elif z == 4 :
    #     #BCA 
    ...
    #     image_to_string_result = pytesseract.image_to_string(image, lang="ind+eng",config="--psm 3 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '")
    if isDebug:
...
def ProcessTesseract_MaximumConfidenceLevel1(img, arrayThreshold, psm, lang):
    if isDebug:
    ...
    for i in arrayThreshold:
        ...
        if psm !='' and lang != '':
                img_data = pytesseract.image_to_data(threshold, lang=str(lang),config='--psm ' + str(psm) + " --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        else :
            message = 'No Language or PSM parameter on ProcessTesseract_MaximumConfidenceLevel1'
            PrintLog(message)
            

        # img_data = pytesseract.image_to_data(threshold, lang=str(lang1),config='--psm ' + str(psm) + " --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        # elif lang1 != '' and lang2=='' and psm != '':
        #     img_data = pytesseract.image_to_data(threshold, lang=str(lang1),config='--psm ' + str(psm) + " --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        # if z==1:
        #     img_data = pytesseract.image_to_data(threshold, lang="ind",config="--psm 6 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        # elif z==2:
        #     img_data = pytesseract.image_to_data(threshold, lang="ind",config="--psm 3 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        # elif z==3:
        #     #BNI TYPE 1
        #     img_data = pytesseract.image_to_data(threshold, lang="ind_w",config="--psm 6 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        # elif z==4:
        #     #BCA
        #     #BNI TYPE 2 (to get the date and description properly)
        #     img_data = pytesseract.image_to_data(threshold, lang="ind+eng",config="--psm 4 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        # else :
        #     #BNI TYPE 2 (to get the mutation, debt/credit desc, and balance properly)
        #     img_data = pytesseract.image_to_data(threshold, lang="ind+eng",config="--psm 3 --oem 3 -c tessedit_char_whitelist=0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/-.,:' '", output_type='data.frame')
        img_data = img_data.dropna()
        total = img_data.sum(axis=0)
        count = img_data.count(axis=0)
        avgConf = total.conf / count.conf
...
#         post_status_to_webapp = requests.post(URL_Success,{'nama_file':file_name, 'conf_lv':Conf , 'calculation':BalanceChecking })
#         print("post success")
...
#         # post_status_to_webapp = requests.post('http://34.124.140.127:80/index.php/panggilAku/receive',{'filename':file_name},files=myfiles)
#         post_status_to_webapp = requests.post('http://34.124.150.81:80/index.php/panggilAku/receive',{'filename':file_name},files=myfiles)
#         if isDeploy:
#             list_path.append(process_filepath)
...
#             output_filepath = PROCESS_FOLDER_PATH + file_name_no_ext + "_" + str(i) + '.jpg'
#             output_filepath2 = PROCESS_FOLDER_PATH + file_name_no_ext + "_" + str(i) + '.jpg'
#             send_webapp = '/var/www/html/maybank-ocr/uploads/static/' + file_name_no_ext + "_" + str(i) + '.jpg'
#             page.save(output_filepath, 'JPEG')
#             page.save(output_filepath2, 'JPEG')
#             file_name = os.path.basename(filepath)
...
def ConvertFileToImageByChris(filepath, dpi):
    ...
    if filepath.lower().endswith(('.png', '.jpg', '.jpeg','.JPG')):
        ...
        image = cv2.imread(filepath)#[...,::-1]
        img.append(image)
        cv2.imwrite(process_filepath, img[0]) #tambahin ini
        file_name = os.path.basename(filepath)
        myfiles = {'image' : open(process_filepath, 'rb')}
        # post_status_to_webapp = requests.post('http://34.124.140.127:80/index.php/panggilAku/receive',{'filename':file_name},files=myfiles)
        # post_status_to_webapp = requests.post('http://34.124.150.81:80/index.php/panggilAku/receive',{'filename':file_name},files=myfiles) #oracle
        post_status_to_webapp = requests.post(URL_Receive,{'filename':file_name},files=myfiles) #postgre
        list_path.append(process_filepath)
...
def Post_Success(OUTPUT_JSN_FOLDER_PATH, file_name, Conf, BalanceChecking):
    try:
        ...
        # post_status_to_webapp = requests.post('http://34.124.140.127:80/index.php/panggilAku/receive',{'filename':file_name,'calculation':BalanceChecking, 'json':contents })
        # post_status_to_webapp = requests.post('http://34.124.150.81:80/index.php/panggilAku/receive',{'filename':file_name,'calculation':BalanceChecking, 'json':contents }) #oracle
        post_status_to_webapp = requests.post(URL_Receive,{'filename':file_name,'calculation':BalanceChecking, 'json':contents }) #postgre
        print("post success")
        timestamp_collector(time.time() - start_time, "Post_Success")
    except:
        Error_Handling(file_name,"Post Json Failed")
