...
def ConvertFileToImageByChris(filepath):
    ...
    if filepath.lower().endswith(('.png', '.jpg', '.jpeg','.JPG')):
        ...
        post_status_to_webapp = requests.post('http://35.198.224.9:80/index.php/panggilAku/receive',{'filename':file_name},files=myfiles) #postgre
        ...
        for i in range(0,250):
            try:
                ...
                PrintLog(message)
                break
        PrintLog('len img from function: ' + str(len(img)))
        try:
            if len(img) == 0:
                PrintLog('1. len img == 0')
                _, img_ = cv2.imreadmulti(filepath, [], cv2.IMREAD_ANYDEPTH)
                file_name_no_ext = os.path.splitext(file_name)[0]
                i=0
                for page in img_:
                    output_filepath = PROCESS_FOLDER_PATH + file_name_no_ext + '.jpg'
                    cv2.imwrite(output_filepath,page)
                    image = cv2.imread(output_filepath)
                    img.append(image)
                    send_webapp = '/var/www/html/maybank-ocr/uploads/static/' + file_name_no_ext + "_" + str(i) + '.jpg'
                    try:
                        ...
                        PrintLog(message)
                    i = i + 1
                PrintLog('2. len img from function: ' + str(len(img)))
        except:
            message = "tiff cannot be read"
            PrintLog(message)
